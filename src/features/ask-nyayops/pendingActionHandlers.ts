...
import { clearStagedAttachment, getStagedAttachment } from './chatAttachmentStore'
...
export function idFromWouldAffect(pa: PendingAction, prefix: string): string {
 ...
 * an editable draft, not a blind re-send of after_state. ApprovalsPage's
 * "Approve" click reuses this exact map (via executeApprovedAction below) so
 * a second approver's confirm calls the same real endpoint a normal
 * PendingActionCard confirm would have - one execution path, not two that
 * could drift apart. */
export const HANDLERS: Record<string, ActionHandler> = {
  ...
  // 'case.assign'/'case.reassign' are deliberately absent - free-text name
  // resolution in chat kept failing on ambiguous/misheard names (Improvements
  // v1 doc-adjacent fix), so they get their own searchable-picker UI
  // (AssignCaseConfirmCard in PendingActionCard.tsx), not a blind re-send of
  // after_state.
  'case.status_update': {
  ...
  'document.create': {
    label: 'Add document',
    // The file dragged/picked into ChatInputBar (chatAttachmentStore) - staged
    // client-side since the chat tool-calling loop never carries file bytes.
    execute: async (pa) => {
      const file = getStagedAttachment()
      if (!file) throw new Error('Attach a file first, then confirm this again.')
      const res = await createUploadUrl({
        case_id: pa.after_state.case_id as string,
        title: pa.after_state.title as string,
        doc_type: pa.after_state.doc_type as string,
        filename: file.name,
  ...
  'document.create_version': {
    ...
    execute: async (pa) => {
      const file = getStagedAttachment()
      if (!file) throw new Error('Attach a file first, then confirm this again.')
      const res = await createDocumentVersion(idFromWouldAffect(pa, 'document'), {
        filename: file.name,
        mime_type: file.type || 'application/octet-stream',
        file_size_bytes: file.size,
        change_note: pa.after_state.change_note as string | undefined,
      })
      await uploadFileBytes(res.upload_url, file)
      await confirmUpload(res.document_id, crypto.randomUUID())
      clearStagedAttachment()
  ...
  // (InviteUserConfirmCard in PendingActionCard.tsx), not a blind re-send of
  // after_state.
  'user.deactivate': {
...
/** T3 actions whose blast radius is the whole org/branch/role, not one
 * record - implementation plan §7.1's "for the widest-radius actions, a
 * second approver". Every other T3 action_type still gets the consequence-
 * restatement step in PendingActionCard, just not this extra gate. Mirrors
 * backend v1's SECOND_APPROVAL_ACTION_TYPES
 * (application/services/assistant_approvals.py) - keep the two in sync if
 * that set ever changes. */
export const SECOND_APPROVAL_ACTION_TYPES = new Set([
  'organization.freeze',
  'branch.delete',
  'role.delete',
])

/** ApprovalsPage's "Approve" click reuses HANDLERS so the second approver's
 * confirm calls the same real REST endpoint a normal PendingActionCard
 * confirm would have. approval carries no notify_on_approval (PendingAction-
 * only plumbing no HANDLERS.execute reads), so it's filled in empty just to
 * satisfy the shared type. */
export async function executeApprovedAction(approval: PendingApproval): Promise<void> {
  const handler = HANDLERS[approval.action_type]
  if (!handler?.execute) {
    throw new Error(`${humanize(approval.action_type)} can't be completed from here.`)
  }
  await handler.execute({ ...approval, notify_on_approval: [] })
}
