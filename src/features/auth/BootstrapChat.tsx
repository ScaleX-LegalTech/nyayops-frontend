...
import { useSpeakReplies } from '@/features/ask-nyayops/useSpeech'
import { VoiceControls } from '@/features/ask-nyayops/VoiceControls'
...
interface ChatTurn {
  ...
  /** True for a user turn spoken rather than typed - see the "you said" banner
   * in BootstrapRegistrationConfirm below (Improvements v1 doc §3.5/§8.2 item
   * 7: a misheard org name/email here is expensive, since confirming creates
   * the tenant). */
  viaVoice?: boolean
}
...
interface BootstrapRegistrationConfirmProps {
  ...
  /** The spoken transcript that led to this draft, only when it came from
   * voice input - shown so a misheard name/email is caught here, not after
   * the tenant's already created. */
      ...
      <CardBody className="flex flex-col gap-3">
        {voiceTranscript && (
          <p className="rounded-control bg-surface-muted px-3 py-2 text-xs text-ink-muted">
            You said: <span className="text-ink">&ldquo;{voiceTranscript}&rdquo;</span>
          </p>
        )}
...
export function BootstrapChat({ onRegister, registering, registerError }: BootstrapChatProps) {
  ...
  const speech = useSpeakReplies('bootstrap')

  /** Shared by the form's submit and VoiceControls' onTranscript - stopping a
   * recording submits it immediately, the same way typed text does on Enter
   * (matches Ask NyayOps' own chat, see useSpeech.ts). */
  async function sendMessage(rawMessage: string, viaVoice = false) {
    ...
    setTurns((prev) => [
      ...prev,
      { id: crypto.randomUUID(), role: 'user', content: message, viaVoice },
    ])
    setInput('')
    setLoading(true)
    setError(null)
    const fillerTimer = speech.enabled ? setTimeout(() => speech.playFiller(), 900) : null
    try {
      ...
      if (speech.enabled) speech.speak(response.reply)
    } catch (err) {
    ...
    <div className="flex h-[28rem] flex-col overflow-hidden rounded-card border border-border bg-surface">
                ...
                <div className={cn('max-w-[85%] min-w-0', turn.role === 'user' && 'items-end')}>
                  ...
                  {turn.pendingAction && (
                    <BootstrapRegistrationConfirm
                      ...
                      voiceTranscript={
                        turns[index - 1]?.viaVoice ? turns[index - 1]?.content : undefined
                      }
                    />
                  )}
                </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2 border-t border-border p-3">
        ...
        <VoiceControls
          ...
          onToggleSpeakReplies={speech.toggle}
          loading={loading}
        />
